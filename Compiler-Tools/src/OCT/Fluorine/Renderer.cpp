...
#include <iostream>
#include <sstream>
using namespace OCT::Fluorine;

void OCT::Fluorine::Renderer::subSingles(std::string & str, std::shared_ptr<Data> data)
{
	...
	while (true) {
		...
		OCT::Regex::Match ms;
		auto single_tag_found = OCT::Regex::search("{{[a-zA-Z0-9_]+}}", str, ms, last_pos);
		if (single_tag_found)
		{
			//found single tag
			//update last pos
...
std::string OCT::Fluorine::Renderer::expandMulti(const std::string & str, std::shared_ptr<Data> data, std::size_t index)
{
	std::string result = "";

	//search for multi tag
	OCT::Regex::Match ms_start, ms_end;
	auto start_tag_found = OCT::Regex::search("{{#[a-zA-Z0-9_]+}}", str, ms_start, index);
	if (start_tag_found)
	{
		...
		auto end_tag_found = OCT::Regex::search(regex, str, ms_end, ms_start.end);
		if (end_tag_found)
		{
...
std::string OCT::Fluorine::Renderer::doWork(std::string & str, std::shared_ptr<Data> data)
{
...
void OCT::Fluorine::Data::addSingle(const std::string & key, const std::string & value)
{
	m_singleTable[key] = value;
}

void OCT::Fluorine::Data::addMulti(const std::string & key, std::shared_ptr<Data> table)
{
	table->m_parent = shared_from_this();
	m_multiTables[key].push_back(table);
}

std::string OCT::Fluorine::Data::getSingle(const std::string & key)
{
...
void OCT::Fluorine::Data::getMulti(const std::string & key, std::vector<std::shared_ptr<Data>>& out)
{
