...
API Token OCT::Lexer::skip_token()
{
	Token result;
	result.isInvalid = false;
	result.isEOF = false;
...
API Token OCT::Lexer::make_token(std::string tagName, std::function<bool(InputStreamPtr, Token&)> eventFunction)
{
	Token result;
	result.tag = tagName;
...
API Token OCT::Lexer::make_token(std::function<bool(InputStreamPtr, Token&)> eventFunction)
{
